...
const notificationDefaults = channels.reduce(
	(defaults: Record<string, any>, channel) => {
		defaults[channel] = {
			type: Boolean,
			default: true,
		};
...
const SettingsSchema = new Schema<SettingsDocument>({
	...
	notification: types.reduce(
		(
			typeObj: Record<string, typeof notificationDefaults>,
			notificationType,
		) => {
...
export default model<SettingsDocument, SettingsModel>(
	'Settings',
	SettingsSchema,
);
