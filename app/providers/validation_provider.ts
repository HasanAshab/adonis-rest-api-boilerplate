...
export default class ValidationProvider {
  ...
  private registerMessagesProvider() {
    vine.messagesProvider = new SimpleMessagesProvider(
      this.app.config.get('validator.customMessages')
    )
  }

  private registerPasswordStrategies() {
    passwordStrategy.register('standard', async () => {
      const { default: StandardPasswordStrategy } = await import(
        '#app/validation/rules/password/strategies/standard_password_strategy'
      )
      return new StandardPasswordStrategy()
    })

    passwordStrategy.register('complex', async () => {
      const { default: ComplexPasswordStrategy } = await import(
        '#app/validation/rules/password/strategies/complex_password_strategy'
      )
      return new ComplexPasswordStrategy()
    })

    passwordStrategy.register('weak', async () => {
      const { default: WeakPasswordStrategy } = await import(
        '#app/validation/rules/password/strategies/weak_password_strategy'
      )
