...
interface HasPolicyDocument<TPolicy extends Policy<Document>> extends Document {
	can<Action extends keyof TPolicy>(
		action: Action,
		target: Parameters<Policy[Action]>[1],
	): Promise<boolean>;
	cannot<Action extends keyof TPolicy>(
		action: Action,
		target: Parameters<Policy[Action]>[1],
	): Promise<boolean>;
}
...
export default function HasPolicy(schema: Schema, Policy: constructor) {
	...
	schema.methods.can = async function (action: string, target: Document) {
		...
		return filter === null ? await policy[action](this, target) : filter;
	};
