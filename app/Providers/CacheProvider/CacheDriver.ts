...
export default abstract class CacheDriver {
  ...
  abstract put<T extends CacheData, U extends boolean>(
    key: string,
    data: T,
    expiry?: number,
    returnSerialized?: U
  ...
  async remember(key: string, expiry: number, resolver: Resolver) {
    return (await this.get(key)) ?? (await this.put(key, await resolver(), expiry))
  }

  async rememberSerialized(key: string, expiry: number, resolver: Resolver) {
    return (await this.get(key, false)) ?? (await this.put(key, await resolver(), expiry, true))
  }

  async rememberForever(key: string, resolver: Resolver) {
    return (await this.get(key)) ?? (await this.put(key, await resolver()))
  }

  async rememberSerializedForever(key: string, resolver: Resolver) {
    return (await this.get(key, false)) ?? (await this.put(key, await resolver(), undefined, true))
  }
