...
export default class MongooseProvider {
	...
	private async connect() {
		const { url, options, connectOnBackground, syncIndexes } =
			Config.get('mongoose');
		...
		if (connectOnBackground) {
			mongoose
				...
				.then(() => {
					...
					syncIndexes &&
						mongoose.syncIndexes().catch(() => {
							Logger.warn(
								'Error occured while connecting to database and syncronizing indexes. reason\n, %o',
								err.stack,
							);
						});
				})
	...
	private async registerUserProvider() {
		const { default: MongoDBAuthProvider } = await import(
			'./MongoDBAuthProvider'
		);
