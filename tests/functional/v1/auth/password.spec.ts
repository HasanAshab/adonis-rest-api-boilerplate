...
test.group('Auth/Password', (group) => {
	...
	test("Shouldn't send reset email of social account", async ({
		client,
		expect,
	}) => {
	...
	test('should reset password', async ({ client, expect }) => {
		const token =
			await new ForgotPasswordNotification().createForgotPasswordToken(user);
	...
	test("shouldn't reset password with invalid token", async ({
		client,
		expect,
	}) => {
		const password = 'Password@1234';
